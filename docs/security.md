...
`files[0].name` or exact input value. If Dice re-renders that input after selection, the exact
filename must instead appear in exactly one current Easy Apply form; global page text is never
upload evidence. A click is never considered success; Dice must display a visible, scoped
